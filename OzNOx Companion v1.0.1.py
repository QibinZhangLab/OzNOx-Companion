        ...
        if matrix_index == (raw_data_frame_size - 1):
            ...
            if matrix_row_ms_level == ms_level:
                ...
                if rt_floor <= matrix_row_rt <= rt_ceiling and good_precursor:
                    ...
                    temp_elements_array = []
                    for matrix_row_spectrum_element in matrix_row_spectrum_elements:
                        if matrix_row_spectrum_element.find(':') > 0:
                            temp_elements_array += [matrix_row_spectrum_element]
                    matrix_row_spectrum_elements = temp_elements_array
...
def oznox_script_7():
    ...
    while not input_done:
        ...
        if not exists(csv_directory):
            ...
            if param_files_pass:
                ...
                try:
                    script_6_output_frame = pd.read_csv(script_6_output_file, low_memory=False)
                except Exception as error_message:
                    param_files_pass = False
                    print(error_message)
            ...
            if param_files_pass:
                ...
                for sample_definitions_group in sample_definitions_groups:
                    ...
                    has_ozid_only = False
                    has_ozid = False
                    final_ref_samples_1 = None
                    final_ref_samples_2 = None
                    final_ref_samples_3 = None
                    final_ref_samples_4 = None
                    for unique_group_and_only_oznox_samples in unique_groups_and_only_oznox_samples:
                        ...
                        if ref_group == sample_definitions_group and len(ref_samples_1) > 0:
                            has_oznox = True
                            has_oznox_only = True
                            final_ref_samples_1 = copy.deepcopy(ref_samples_1)
                    if not has_oznox:
                        for unique_group_and_oznox_samples in unique_groups_and_oznox_samples:
                            ...
                            if ref_group == sample_definitions_group and len(ref_samples_2) > 0:
                                has_oznox = True
                                final_ref_samples_2 = copy.deepcopy(ref_samples_2)
                    for unique_group_and_only_ozid_samples in unique_groups_and_only_ozid_samples:
                        ...
                        if ref_group == sample_definitions_group and len(ref_samples_3) > 0:
                            final_ref_samples_3 = copy.deepcopy(ref_samples_3)
                            has_ozid = True
                            has_ozid_only = True
                    if not has_ozid:
                        for unique_group_and_ozid_samples in unique_groups_and_ozid_samples:
                            ...
                            if ref_group == sample_definitions_group and len(ref_samples_4) > 0:
                                has_ozid = True
                                final_ref_samples_4 = copy.deepcopy(ref_samples_4)
                    ...
                    if has_ozid_only:
                        print('If/when User RT is not available, the following samples will be queried: ')
                        for ref_sample in final_ref_samples_3:
                            print(ref_sample)
                    elif has_ozid:
                        print('If/when User RT is not available, the following samples will be queried: ')
                        for ref_sample in final_ref_samples_4:
                            print(ref_sample)
                    if has_oznox_only:
                        print('To assign double-bond locations, the following samples will be queried: ')
                        for ref_sample in final_ref_samples_1:
                            print(ref_sample)
                    elif has_oznox:
                        print('To assign double-bond locations, the following samples will be queried: ')
                        for ref_sample in final_ref_samples_2:
                            print(ref_sample)
    ...
    previous_percent_complete = -1
    for i in range(script_6_output_frame_size):
        current_percent_complete = int(100 * (i + 1) / script_6_output_frame_size)
        if current_percent_complete > previous_percent_complete:
            previous_percent_complete = current_percent_complete
            print('Alignment and data preparation is ' + str(current_percent_complete) + '% finished')
    ...
    for parent_id in parent_ids:
        ...
        if len(unique_individual_oznox_keys) > 0:
            ...
            for unique_oznox_key_set in unique_oznox_key_sets:
                ...
                for group_i in sample_definitions_groups:
                    groups_and_rts_counts_spectrum[group_i] = [-1.0, 0.0, '', 0.0]
...
def main():
    ...
    version_number_string = 'v1.0.1'
    version_description_string = 'Post-Publication Version: bug fixes, quality of life improvement'
